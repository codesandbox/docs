...
Support and maintenance for CodeSandbox for iOS are temporarily suspended. 

The CodeSandbox editor has received several significant updates in recent months that have caused significant issues for a number of iOS users. As a result of these issues, we have concluded that the iOS app no longer meets the high standards we set for our products. While we explore solutions for these issues, we have decided to de-list CodeSandbox for iOS from the App Store. This does not remove the app from your device or prevent you from opening it if you already have it installed.

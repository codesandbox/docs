...
CodeSandbox for iOS is no longer supported.

Because we were unable to update the iOS experience at the rate required to keep up with the changes happening on the web editor, we have decided to deprecate the iOS app and remove it from the App Store. This will not remove the app from your device or prevent you from opening it if you already have it installed.

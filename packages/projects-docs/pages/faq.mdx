...
The CodeSandbox editor has received several significant changes that have resulted in issues for iOS users. As a result, we concluded that the iOS app would stop meeting the high standards we set for our products, so we decided to de-list CodeSandbox for iOS from the App Store. This does not remove the app from your device or prevent you from opening it if you already had it installed.

We will send further updates about the state of CodeSandbox for iOS when this changes.

...
A workspace can control the use of virtual machines in several ways. The functionalities described below allow workspace admins to increase predictibility of VM usage and credit spend.
...
Each workspace has a range of VM sizes to select from. The range of options varies depending on the subscription tier of the workspace. 
By default, all Devboxes and repos will run on [Micro](/learn/credit-usage/credits). The default can be changed in the VM [settings page](codesandbox.io/t/vm_settings) in the workspace portal. 
...
To set a Devbox or repositority VMs, you can create a rule. This is helpful for ensuring that heavy compute projects get the power they need without increasing VM usage across the other projects.

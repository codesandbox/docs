...
A synced template URL is essentially the same as the GitHub URL, but `github.com` is replaced with a CodeSandbox path.

We use some heuristics to detect if a given repository should be opened as a synced Sandbox or a synced Devbox.

In a nutshell:

- To create a synced template as a Devbox, `github.com` is replaced with `codesandbox.io/p/devbox/github`.
- To create a synced template as a Sandbox, `github.com` is replaced with `codesandbox.io/p/sandbox/github`.

For example, the following URL will open a synced template as a Devbox for the Astro Blog examples folder:

`https://codesandbox.io/p/devbox/github/withastro/astro/main/examples/blog`

We have also introduced three helpers to make it easier to create synced templates, as described below.

...
To run additional services using the Docker compose integration, you can create a file `.codesandbox/docker-compose.yml`. This file will be used to run `docker compose up` on start of branch/sandbox. This can be used to run other services needed in the development environment like databases, proxies, etc.
...
You can use docker commands in the terminal to exec into the containers started with this method. To access the containers with the `docker compose` command either change the directory into `.codesandbox` or specify the location of the file using `docker compose -f .codesandbox/docker-compose.yml`.
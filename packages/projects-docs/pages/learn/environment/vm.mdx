...
CodeSandbox Repositories and Devboxes run on [microVM technology](https://codesandbox.io/blog/how-we-clone-a-running-vm-in-2-seconds). Each branch and Devbox gets its own microVM and their resources (shown below) are set to each one of these instances.
...
But we can go higher, up to 32vCPUs, 64GB memory and 256GB storage. To upgrade to higher specs, you can ugrade to one of our [Pro plans](https://codesandbox.io/pro).

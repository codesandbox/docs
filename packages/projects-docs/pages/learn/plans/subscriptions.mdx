...
Note that if the free credits run out before the end of the billing cycle, all VM Sandboxes and Repositories will enter into a [frozen state](/learn/plans/pricing-faq#my-vm-sandbox-or-repository-is-frozen-what-does-that-mean).

In addition to the monthly free credits, the Free tier also provides access to 20 Sandboxes for the entire workspace.
Unlike VM Sandboxes and repositories, Browser Sandboxes do not run on credits. Instead, there are multiple options for accessing Sandboxes at a flat rate. This is described more in the following section.
...
- You need additional VM storage (per each individual VM Sandbox and Repository branch).

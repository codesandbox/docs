...
Tasks can have their ports assigned, normally we try to detect ports assigned to a task, but there are three advantages to define them manually using the `preview` field:

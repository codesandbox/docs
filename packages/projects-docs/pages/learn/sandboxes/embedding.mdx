...
Embeds are currently supported as a beta feature for cloud sandboxes and repositories. While these provide fewer customization options as compared to browser sandbox embeds, they are much more powerful and support any programming language, not just JavaScript.

<Tabs tabs={["Cloud Sandboxes & Repositories", "Browser Sandboxes"]}>
    <WrapContent>

Cloud sandboxes ir repositories embeds are a great solution if you want to show examples of applications that go beyond front-end JavaScript (namely, other programming languages).
...
<Callout>
Embeds for cloud sandboxes and repositories are currently in beta.
</Callout>

## Generate an Embed URL

You can generate a URL to embed by clicking the `Share` button inside the editor of a cloud sandbox or a repository and then clicking on 'Copy embed code'.

This will give you HTML code with an iFrame that you can paste in your content.

`Screenshot of the "Copy embed code"`

## Limitations

- Is there any expected limit to the number of concurrent viewers? Do we support thousands at the same time?
- Are there any differences between free and pro users when it comes to embeds?
- Which V1 embed features are missing in V2?
  - customization
  - others?

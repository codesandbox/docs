...
```ts
// Optionally start the dev server as a background command
const devCommand = client.commands.runBackground('npm run dev')
...
// When you no longer need the dev server, kill it
devCommand.kill()
```

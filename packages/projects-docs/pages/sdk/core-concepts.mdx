...
Pay for long term persistence and predictable resume times.

- Sandboxes will never be archived when hibernated, you control when to delete them
- Resuming a hibernated Sandbox will take < 5 seconds
- Safely create Sandboxes from other Sandboxes
...
- Use a proxy through your own server to improve interactions with hosts
...
- Sandbox continues from snapshot in 0.5-2s or 10-60s if archived

### Deleting

- Push persistence to git/db using the SDK
- Delete the Sandbox using the SDK

### Resume from deletion

- A user decides to come back

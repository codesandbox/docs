...
Every sandbox has a persistent file system in the default working directory: `/project/sandbox`. All files saved in `/project/sandbox` have `git` source control and will be persisted between reboots. Whenever we hibernate or shutdown a sandbox, we create a commit to track the filesystem of the sandbox.
...
```ts
const { downloadUrl } = await session.fs.download("./");
...
```ts
const watcher = await session.fs.watch("./", { recursive: true, excludes: [".git"] });

...
Create a new branch and set its upstream, ready to push.

```ts
await client.git.createBranch("some-branch")
```

### Checkout

Checkout any git reference.

```ts
await client.git.checkout("some-branch-or-sha")
```

...
- Not just files, but running processes ready for continuation

Templates ensure consistent, fast Sandbox creation and eliminate the need to install dependencies and even start processes.
...
1. Create your template structure locally
...
1. Instantiate new Sandboxes from templates (~1-3 seconds)
2. Establish secure sessions with appropriate permissions
3. Interact with the Sandbox using our clients

### 3. Lifecycle Management

1. Resume Sandboxes when user sessions start (~0.5-2 seconds)
2. Hibernate Sandboxes when user sessions end
3. Delete Sandboxes when no longer in use

...
await sdk.sandbox.hibernate(sandbox2.id);

// Resume the sandbox
const resumedSandbox = await sdk.sandbox.resume(sandbox2.id)
```

<Callout>
The `connect` method connects you to the sandbox with a websocket connection, but you can also use `createBrowserSession` or `createRestClient` for other types of connections.
</Callout>

...
const sandbox = await sdk.sandbox.fromTemplate()

// Create a client to interact with the environment
...
// Resume the sandbox
const newClient2 = await sandbox2.connect()
```

<Callout>
The `createWebSocketSession` method connects you to the sandbox with a websocket connection, but you can also use `createBrowserSession`, `createRestSession` or `createSshSession` for other types of sessions.
</Callout>

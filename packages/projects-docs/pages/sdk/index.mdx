...
session = await sandbox2.connect()

// Sandbox 2 will have the same processes running as sandbox 1
const portInfo2 = await session.ports.waitForPort(3000);
...
<Callout>
The `connect` method connects you to the sandbox with a websocket connection, but you can also use `rest`, `json` or `ssh` for other types of connections.
</Callout>

...
If you call `sdk.sandboxes.resume(id)` or a network request is made to the Sandbox, we restore the snapshot. This allows you to continue from exactly where you left off.

### Manual Hibernation

We highly recommend taking full control of the lifecycle by manually hibernating sandboxes. Even though a timeout is easier to implement, it is not as efficient and risks creating janky UX and higher costs.
...
app.get('/project/:projectId/*', async (req, res) => {
  ...
  if (project.sandboxId) {
    ...
    const hostUrl = await client.getHost();
...
You should use your own persistence layer to track VM sessions and sandbox states. These monitoring methods are designed to get additional information about a sandbox and its current running state, not as your primary source of truth for lifecycle management.

...
You should use your own persistence layer to track VM sessions and sandbox lifecycle states. This ensures that you are able to create the optimal user experience with minimal costs. The exact lifecycle management depends on how you approach persistence.

<Callout type="warning">
Even though `hibernationTimeoutSeconds` and `wakeupConfig` is currently provided, they are _deprecated_. 
</Callout>

## Default Persistence (free)

With default persistence hibernated Sandboxes will be archived after 2-7 days. If your user experience allows for resume times of 10-60 seconds, you can lean on `hibernate` and `resume` to only manage the cost of running VMs.
...
- **Host request**: Use a proxy to resume the Sandbox on HTML requests and hibernate on request debounce. Read more in [Hosts](./sandbox-hosts)
...
You can choose to not rely on our persistence at all. By rather using `delete` instead of `hibernate` you get minimum cost and optimal control of Sandbox resumes.

When you want to stop using the Sandbox, push any changes to db/git persistence. Then `delete` the Sandbox. When you want to resume the Sandbox you rather `create` a new Sandbox from the template and pull db/git persistence back into the Sandbox.
...
With long term persistence there is no archiving of hibernated Sandboxes after 2-7 days. That means you will get reliable resumes regardless of the age of the Sandbox. There will be a cost though, so you should still delete Sandboxes that you consider ephemeral (Like AI suggestions). Long term persistence also enables you to reliably create a Sandbox from an existing hibernated Sandbox.

<Callout type="warning">
There is still a scenario where a cluster issue requires recover from a backup. In this case it will be a `CLEAN` boot.
</Callout>
...
- **Delete Sandboxes** - With custom or long term persistence you should delete Sandboxes to manage cost

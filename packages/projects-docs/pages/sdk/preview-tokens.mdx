...
```ts
const previewToken = await sdk.sandbox.previewTokens.create('some-sandbox-id', {
...
const signedUrl = sdk.sandbox.previewTokens.getSignedPreviewUrl(3000, previewToken.token)
...
export const POST = async ({ params, body }) => {
  const previewToken = await sdk.sandbox.previewTokens.create(params.id);

  return sdk.sandbox.previewTokens.getSignedPreviewUrl(body.port, previewToken.token)
}

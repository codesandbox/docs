...
You can obtain a preview token by calling `sdk.previewTokens.create`. You can also set a custom expiration time for the token:
...
const signedUrl = sdk.previewTokens.getSignedPreviewUrl(3000, previewToken.token)
...
export const POST = async ({ params, body }) => {
  ...
  return sdk.previewTokens.getSignedPreviewUrl(body.port, previewToken.token)
}

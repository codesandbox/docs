...
Design your UX for variable resume times. Sandboxes that have been dormant for a 4 days will perform a clean bootup, which takes significantly longer than a regular resume. Always show appropriate loading states and communicate the expected wait time to users.
...
- **Manage lifecycles** - Do not rely on hibernation timeouts, but actively hibernate and resume
- **Track Sandbox state** - Use your own persistence store to track when Sandboxes where last resumed and hibernate, giving a proactive UX
- **Keeping Sandboxes warm** - When actively managing the lifecycle you can also wake up and hibernate Sandboxes to extend their optimal state for longer

...
Design your UX for variable resume times. Sandboxes that have been dormant for a week will perform a clean bootup, which takes significantly longer than a regular resume. Always show appropriate loading states and communicate the expected wait time to users.

- **Show loading indicators** - Resume times vary from 0.5-2 seconds (regular) to 10-60 seconds (clean bootup)
- **Communicate status clearly** - Tell users when a sandbox is performing a clean bootup vs. regular resume
- **Handle timeouts gracefully** - Implement proper error handling for resume failures
- **Consider proactive warming** - Resume frequently-used sandboxes before users need them
- **Monitor resume patterns** - Track which sandboxes need clean bootups to optimize user experience
- **Handle both resume types** - Always account for both regular resumes and clean bootups in your application logic
- **Check bootup type** - Use `sandbox.bootupType` to determine if special handling is needed
- **Verify connectivity** - Ensure your application can handle the brief reconnection period
</Callout>

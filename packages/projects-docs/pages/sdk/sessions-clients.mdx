...
await client.fs.writeTextFile('test.txt', 'Hello World'); // This will throw an error.
await client.fs.readTextFile('test.txt'); // This will work.

// I can also create sessions with write access
const client2 = await sandbox.connect({
  id: 'some-user-reference',
  permission: 'write',
});
await client2.fs.writeTextFile('test.ext', 'Hello World'); // This will work
```
...
await client.fs.writeTextFile({ path: 'test.ext', content: 'Hello World' })
```

...
const shell = session.shells.create('bash')
...
const command = session.shells.run("npm install");
...
// Run JavaScript code
const jsResult = await session.shells.js.run(`
  console.log("Hello from Node.js!");
`);

// Run Python code
const pythonResult = await session.shells.python.run(`
...
// Get all shells
const shells = await session.shells.getShells();
...
const shell = session.shells.run("npx -y serve .");

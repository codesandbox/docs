...
The `restartOn` option is deprecated and should not be used. You should manage the lifecycle of any processes from your server application to avoid unpredictable behavior or accidentally sub optimize your templates. For example after a `create` you can restart the dev server on the Sandbox by connecting to it.
</Callout>

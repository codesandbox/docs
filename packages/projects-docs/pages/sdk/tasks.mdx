...
// Get all tasks
const tasks = await session.tasks.getTasks();
...
if (task.ports.length > 0) {
  ...
  console.log(`Preview available at: ${port.url}`);
}
```
...
// Get a specific task
const task = await session.tasks.getTask("build");
if (task) {
...
// Get the dev task
const task = await session.tasks.getTask("dev");
if (!task) {
  throw new Error("Dev task not found");
}

// Run the task
await session.tasks.runTask(task.id);

// If the task has a preview port configured
if (task.preview?.port) {
  // Wait for the port to open
  const portInfo = await session.ports.waitForPort(task.preview.port);
  console.log(`Dev server ready at: ${portInfo.url}`);
}
```
...
session.ports.onDidPortOpen((portInfo) => {
  ...
  if (task) {
    console.log(`${task.name} is ready at: ${portInfo.url}`);
  }
});
```

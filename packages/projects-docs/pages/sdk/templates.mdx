...
All sandboxes created by the CodeSandbox SDK are created from a template. Even if you don't pass an argument to `sdk.sandbox.create`, we will fork the "Universal" template to create a new sandbox.
...
You can start from a template by passing the template id to the `sdk.sandbox.create` method.
...
<Callout>
  Want to create your own template? Check out the [Snapshot Builder](/sdk/snapshot-builder) guide to learn how to create a template from your own files using a CLI.
</Callout>

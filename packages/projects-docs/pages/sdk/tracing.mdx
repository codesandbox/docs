...
// Pass tracer to SDK
const sdk = new CodeSandbox({ tracer });
```
...
```ts
const sandbox = await sdk.sandboxes.create('some-template-id'); // Traced as "sandboxes.create"
...
// Terminal management
const terminal = await client.terminals.create('some-template-id'); // Traced as "sandboxClient.terminals.create"
...
const csb = new CodeSandbox({ tracer });
```

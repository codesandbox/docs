...
Create a new file `docker-compose.yml` in `.codesandbox` folder. As an example you can add the following contents in the file to run a nginx server.
...
Now that you've created the `docker-compose.yml` and saved it, you should see a notification pop up, asking you to restart the `docker compose` command. Whenever the `docker-compose.yml` changes, this prompt will show and you can restart docker compose.

![Notification for starting docker compose](/docker-compose-notification.png)

You can press "Yes", and it will run `docker compose up` for you. You should now see a shell dev tool open will docker compose logs. Once the `nginx` image is pulled and run, the port `8080` should open and a notification should show.